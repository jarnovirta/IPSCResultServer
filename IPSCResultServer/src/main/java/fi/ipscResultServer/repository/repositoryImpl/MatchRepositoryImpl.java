...
public class MatchRepositoryImpl implements MatchRepository {
	...
	public List<Match> getMatchListForUser(User user) {
		try {
			String queryString = "SELECT NEW fi.ipscResultServer.domain.Match(m.id, m.name, m.practiScoreId, m.date, "
					+ "m.status, m.user, m.uploadedByAdmin) FROM Match m "
	...
	public List<Match> getFullMatchList() {
		try {
			String queryString = "SELECT NEW fi.ipscResultServer.domain.Match(m.id, m.name, m.practiScoreId, m.date, m.status, m.user, m.uploadedByAdmin) FROM Match m ORDER BY m.date DESC";
			TypedQuery<Match> query = entityManager.createQuery(queryString, Match.class);
